...
void ImGui::TableUpdateLayout( ImGuiTable* table )
{
    ...
    {
        const ImGuiID context_menu_id = ImHashStr( xorstr_( "##ContextMenu" ) , 0 , table->ID );
...
void ImGui::TableSetupColumn( const char* label , ImGuiTableColumnFlags flags , float init_width_or_weight , ImGuiID user_id )
{
    ...
    if ( label != NULL && label[0] != 0 )
    {
        ...
        table->ColumnsNames.append( label , label + base::memory::crc32_strlen( label ) + 1 );
    }
}
...
const char* ImGui::TableGetColumnName( const ImGuiTable* table , int column_n )
{
    if ( table->IsLayoutLocked == false && column_n >= table->DeclColumnsCount )
        return xorstr_(""); // NameOffset is invalid at this point
...
void ImGui::TableBeginCell( ImGuiTable* table , int column_n )
{
    ...
    {
        LogRenderedText( &window->DC.CursorPos , xorstr_("|") );
...
void ImGui::TableMergeDrawChannels( ImGuiTable* table )
{
    ...
    if ( merge_group_mask != 0 )
    {
        ...
        for ( int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE( merge_groups ); merge_group_n++ )
        {
            if ( int merge_channels_count = merge_groups[merge_group_n].ChannelsCount )
            {
                ...
                for ( int n = 0; n < splitter->_Count && merge_channels_count != 0; n++ )
                {
                    ...
                    base::memory::_memcpy( dst_tmp++ , channel , sizeof( ImDrawChannel ) );
                }
            }

            // Make sure Bg2DrawChannelUnfrozen appears in the middle of our groups (whereas Bg0/Bg1 and Bg2 frozen are fixed to 0 and 1)
            if ( merge_group_n == 1 && has_freeze_v )
                base::memory::_memcpy( dst_tmp++ , &splitter->_Channels[table->Bg2DrawChannelUnfrozen] , sizeof( ImDrawChannel ) );
        }
        ...
        base::memory::_memcpy( splitter->_Channels.Data + LEADING_DRAW_CHANNELS , g.DrawChannelsTempMergeBuffer.Data , ( splitter->_Count - LEADING_DRAW_CHANNELS ) * sizeof( ImDrawChannel ) );
    }
}
...
void ImGui::TableHeader( const char* label )
{
    ...
    if ( ( table->Flags & ImGuiTableFlags_Sortable ) && !( column->Flags & ImGuiTableColumnFlags_NoSort ) )
    {
        ...
        {
            ImFormatString( sort_order_suf , IM_ARRAYSIZE( sort_order_suf ) , xorstr_("%d") , column->SortOrder + 1 );
    ...
    if ( text_clipped && hovered && g.HoveredIdNotActiveTimer > g.TooltipSlowDelay )
        SetTooltip( xorstr_( "%.*s") , (int)( label_end - label ) , label );
...
void ImGui::TableDrawContextMenu( ImGuiTable* table )
{
    ...
    if ( table->Flags & ImGuiTableFlags_Resizable )
    {
        if ( column != NULL )
        {
            ...
            if ( MenuItem( xorstr_( "Size column to fit###SizeOne") , NULL , false , can_resize ) )
                TableSetColumnWidthAutoSingle( table , column_n );
        }

        const char* size_all_desc;
        if ( table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && ( table->Flags & ImGuiTableFlags_SizingMask_ ) != ImGuiTableFlags_SizingFixedSame )
            size_all_desc = xorstr_( "Size all columns to fit###SizeAll");        // All fixed
        else
            size_all_desc = xorstr_( "Size all columns to default###SizeAll");    // All stretch or mixed
    ...
    {
        if ( MenuItem( xorstr_( "Reset order") , NULL , false , !table->IsDefaultDisplayOrder ) )
            table->IsResetDisplayOrderRequest = true;
        want_separator = true;
    }
...
static void* TableSettingsHandler_ReadOpen( ImGuiContext* , ImGuiSettingsHandler* , const char* name )
{
    ImGuiID id = 0;
    int columns_count = 0;
    if ( sscanf( name , xorstr_("0x%08X,%d") , &id , &columns_count ) < 2 )
        return NULL;
...
static void TableSettingsHandler_ReadLine( ImGuiContext* , ImGuiSettingsHandler* , void* entry , const char* line )
{
    ...
    if ( sscanf( line , xorstr_("RefScale=%f") , &f ) == 1 ) { settings->RefScale = f; return; }

    if ( sscanf( line , xorstr_("Column %d%n") , &column_n , &r ) == 1 )
    {
        ...
        if ( sscanf( line , xorstr_("UserID=0x%08X%n") , (ImU32*)&n , &r ) == 1 ) { line = ImStrSkipBlank( line + r ); column->UserID = (ImGuiID)n; }
        if ( sscanf( line , xorstr_( "Width=%d%n") , &n , &r ) == 1 ) { line = ImStrSkipBlank( line + r ); column->WidthOrWeight = (float)n; column->IsStretch = 0; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if ( sscanf( line , xorstr_( "Weight=%f%n") , &f , &r ) == 1 ) { line = ImStrSkipBlank( line + r ); column->WidthOrWeight = f; column->IsStretch = 1; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if ( sscanf( line , xorstr_( "Visible=%d%n") , &n , &r ) == 1 ) { line = ImStrSkipBlank( line + r ); column->IsEnabled = (ImU8)n; settings->SaveFlags |= ImGuiTableFlags_Hideable; }
        if ( sscanf( line , xorstr_( "Order=%d%n") , &n , &r ) == 1 ) { line = ImStrSkipBlank( line + r ); column->DisplayOrder = (ImGuiTableColumnIdx)n; settings->SaveFlags |= ImGuiTableFlags_Reorderable; }
        if ( sscanf( line , xorstr_( "Sort=%d%c%n") , &n , &c , &r ) == 2 ) { line = ImStrSkipBlank( line + r ); column->SortOrder = (ImGuiTableColumnIdx)n; column->SortDirection = ( c == '^' ) ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending; settings->SaveFlags |= ImGuiTableFlags_Sortable; }
    }
}
...
static void TableSettingsHandler_WriteAll( ImGuiContext* ctx , ImGuiSettingsHandler* handler , ImGuiTextBuffer* buf )
{
    ...
    for ( ImGuiTableSettings* settings = g.SettingsTables.begin( ); settings != NULL; settings = g.SettingsTables.next_chunk( settings ) )
    {
        ...
        buf->appendf( xorstr_( "[%s][0x%08X,%d]\n") , handler->TypeName , settings->ID , settings->ColumnsCount );
        if ( settings->RefScale != 0.0f )
            buf->appendf( xorstr_( "RefScale=%g\n") , settings->RefScale );
        ...
        for ( int column_n = 0; column_n < settings->ColumnsCount; column_n++ , column++ )
        {
            ...
            if ( !save_column )
                continue;
            buf->appendf( xorstr_( "Column %-2d") , column_n );
            if ( column->UserID != 0 )                    buf->appendf( xorstr_( " UserID=%08X") , column->UserID );
            if ( save_size && column->IsStretch )         buf->appendf( xorstr_( " Weight=%.4f") , column->WidthOrWeight );
            if ( save_size && !column->IsStretch )        buf->appendf( xorstr_( " Width=%d") , (int)column->WidthOrWeight );
            if ( save_visible )                           buf->appendf( xorstr_( " Visible=%d") , column->IsEnabled );
            if ( save_order )                             buf->appendf( xorstr_( " Order=%d") , column->DisplayOrder );
            if ( save_sort && column->SortOrder != -1 )   buf->appendf( xorstr_( " Sort=%d%c") , column->SortOrder , ( column->SortDirection == ImGuiSortDirection_Ascending ) ? 'v' : '^' );
            buf->append( xorstr_( "\n") );
        }
        buf->append( xorstr_( "\n") );
    }
...
void ImGui::TableGcCompactSettings( )
{
    ...
    for ( ImGuiTableSettings* settings = g.SettingsTables.begin( ); settings != NULL; settings = g.SettingsTables.next_chunk( settings ) )
        if ( settings->ID != 0 )
            base::memory::_memcpy( new_chunk_stream.alloc_chunk( TableSettingsCalcChunkSize( settings->ColumnsCount ) ) , settings , TableSettingsCalcChunkSize( settings->ColumnsCount ) );
...
ImGuiID ImGui::GetColumnsID( const char* str_id , int columns_count )
{
    ...
    ImGuiID id = window->GetID( str_id ? str_id : xorstr_("columns") );
    PopID( );
