    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        if (!g.LogEnabled)
        {
            ...
            if (lines_skippable > 0)
            {
                ...
                {
                    const char* line_end = (const char*)base::memory::_memchr(line, '\n', text_end - line);
                    if (!line_end)
                        line_end = text_end;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration(xorstr_("["), xorstr_("]"));
...
{
    return window->GetIDNoKeepAlive(axis == ImGuiAxis_X ? xorstr_( "#SCROLLX") : xorstr_( "#SCROLLY") );
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImGuiID id = window->GetID(xorstr_("#image"));
    PopID();
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (g.LogEnabled)
        LogRenderedText(&label_pos, active ? xorstr_("(x)") : xorstr_("( )"));
    if (label_size.x > 0.0f)
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), xorstr_( "%.0f%%"), fraction * 100 + 0.01f);
        overlay = overlay_buf;
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr( xorstr_( "##ComboPopup"), 0, id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), xorstr_( "##Combo_%02d") , g.BeginPopupStack.Size); // Recycle windows based on depth
...
static bool Items_SingleStringGetter(void* data, int idx, const char** out_text)
{
    ...
    while (*p)
    {
        if (idx == items_count)
            break;
        p += base::memory::crc32_strlen(p) + 1;
        items_count++;
    }
    if (!*p)
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (!items_getter(data, i, &item_text))
            item_text = xorstr_( "*Unknown item*");
...
{
    { sizeof(char),             xorstr_("S8"),   xorstr_("%d"),   xorstr_("%d")    },  // ImGuiDataType_S8
    { sizeof(unsigned char),    xorstr_("U8"),   xorstr_("%u"),   xorstr_("%u")    },
    { sizeof(short),            xorstr_("S16"),  xorstr_("%d"),   xorstr_("%d")    },  // ImGuiDataType_S16
    { sizeof(unsigned short),   xorstr_("U16"),  xorstr_("%u"),   xorstr_("%u")    },
    { sizeof(int),              xorstr_("S32"),  xorstr_("%d"),   xorstr_("%d")    },  // ImGuiDataType_S32
    { sizeof(unsigned int),     xorstr_("U32"),  xorstr_("%u"),   xorstr_("%u")    },
    { sizeof(ImS64),            xorstr_("S64"),  xorstr_("%I64d"), xorstr_("%I64d") },  // ImGuiDataType_S64
    { sizeof(ImU64),            xorstr_("U64"),  xorstr_("%I64u"), xorstr_("%I64u") },
    { sizeof(float),            xorstr_("float"), xorstr_("%.3f"), xorstr_("%f")    },  // ImGuiDataType_Float (float are promoted to double in va_arg)
    { sizeof(double),           xorstr_("double"), xorstr_("%f"), xorstr_("%lf")   },  // ImGuiDataType_Double
};
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        ImGuiContext& g = *GImGui;
        ImFormatString(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), xorstr_( "%.*s%%d%s" ), (int)(fmt_start - fmt), fmt, fmt_end); // Honor leading and trailing decorations, but lose alignment/precision.
        return g.TempBuffer;
#else
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    base::memory::_memcpy(&data_backup, p_data, type_info->Size);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (data_type == ImGuiDataType_S32 && strcmp(format, xorstr_( "%d") ) != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d", read function more details.)
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        value_changed |= DragScalar(xorstr_(""), data_type, p_data, v_speed, p_min, p_max, format, flags);
        PopID();
        PopItemWidth();
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar( xorstr_("##min"), ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar( xorstr_("##max"), ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags min_flags = flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
    bool value_changed = DragInt( xorstr_("##min"), v_current_min, v_speed, min_min, min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    ImGuiSliderFlags max_flags = flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragInt( xorstr_("##max"), v_current_max, v_speed, max_min, max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= SliderScalar(xorstr_(""), data_type, v, v_min, v_max, format, flags);
        PopID();
        PopItemWidth();
...
bool ImGui::CustomSliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, ImVec2 size, ImVec4 color)
{
    return false;
}
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        ImGuiDataTypeTempStorage data_backup;
        base::memory::_memcpy(&data_backup, p_data, data_type_size);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText(xorstr_(""), buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar(xorstr_(""), data_type, p_data, p_step, p_step_fast, format, flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? xorstr_("%08X") : xorstr_("%d");
    return InputScalar(label, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags);
}

bool ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 2, NULL, NULL, xorstr_("%d"), flags);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 3, NULL, NULL, xorstr_("%d"), flags);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 4, NULL, NULL, xorstr_("%d"), flags);
}
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* new_text, int new_text_len)
{
    ...
    if (pos != text_len)
        base::memory::_memmove(text + pos + new_text_len, text + pos, (size_t)(text_len - pos) * sizeof(ImWchar));
    base::memory::_memcpy(text + pos, new_text, (size_t)new_text_len * sizeof(ImWchar));
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    const int new_text_len = new_text_end ? (int)(new_text_end - new_text) : (int)base::memory::crc32_strlen(new_text);
    ...
    if (BufTextLen != pos)
        base::memory::_memmove(Buf + pos + new_text_len, Buf + pos, (size_t)(BufTextLen - pos));
    base::memory::_memcpy(Buf + pos, new_text, (size_t)new_text_len * sizeof(char));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        const int buf_len = (int)base::memory::crc32_strlen(buf);
        state->InitialTextA.resize(buf_len + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        base::memory::_memcpy(state->InitialTextA.Data, buf, buf_len + 1);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                const int clipboard_len = (int)base::memory::crc32_strlen(clipboard);
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    if (buf_dirty)
                    {
                        IM_ASSERT(callback_data.BufTextLen == (int)base::memory::crc32_strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
    ...
    {
        buf_display = hint;
        buf_display_end = hint + base::memory::crc32_strlen(hint);
    }
    ...
    else
    {
        ...
        if (is_multiline)
            ...
            buf_display_end = buf_display + base::memory::crc32_strlen(buf_display);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick(xorstr_("context"), ImGuiPopupFlags_MouseButtonRight);
        ...
        if (BeginPopup(xorstr_("picker")))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                value_changed |= ColorPicker4(xorstr_("##picker"), col, picker_flags, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        InvisibleButton(xorstr_("hsv"), ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    {
        // SV rectangle logic
        InvisibleButton(xorstr_("sv"), ImVec2(sv_picker_size, sv_picker_size));
        if (IsItemActive())
        {
        ...
        InvisibleButton(xorstr_("hue"), ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if (alpha_bar)
    {
        ...
        InvisibleButton(xorstr_("alpha"), ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4(xorstr_("##rgb"), col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(xorstr_("##hsv"), col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(xorstr_("##hex"), col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton( xorstr_( "##preview" ), cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
    if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags_InputMask_))
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text( xorstr_( "#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)" ), cr, cg, cb, cr, cg, cb, col[0], col[1], col[2]);
        else
            Text( xorstr_( "#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)" ), cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text( xorstr_( "H: %.3f, S: %.3f, V: %.3f" ), col[0], col[1], col[2]);
        else
            Text( xorstr_( "H: %.3f, S: %.3f, V: %.3f, A: %.3f" ), col[0], col[1], col[2], col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_inputs && !allow_opt_datatype) || !BeginPopup( xorstr_( "context") ))
        return;
    ImGuiContext& g = *GImGui;
    ImGuiColorEditFlags opts = g.ColorEditOptions;
    if (allow_opt_inputs)
    {
        if (RadioButton( xorstr_( "RGB"), (opts & ImGuiColorEditFlags_DisplayRGB) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (RadioButton( xorstr_( "HSV"), (opts & ImGuiColorEditFlags_DisplayHSV) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (RadioButton( xorstr_( "Hex"), (opts & ImGuiColorEditFlags_DisplayHex) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHex;
    }
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs) Separator();
        if (RadioButton( xorstr_( "0..255"),     (opts & ImGuiColorEditFlags_Uint8) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (RadioButton( xorstr_( "0.00..1.00"), (opts & ImGuiColorEditFlags_Float) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Float;
    }

    if (allow_opt_inputs || allow_opt_datatype)
        Separator();
    if (Button( xorstr_( "Copy as.."), ImVec2(-1, 0)))
        OpenPopup( xorstr_( "Copy") );
    if (BeginPopup( xorstr_( "Copy") ))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), xorstr_( "(%.3ff, %.3ff, %.3ff, %.3ff)") , col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), xorstr_( "(%d,%d,%d,%d)") , cr, cg, cb, ca);
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), xorstr_( "#%02X%02X%02X"), cr, cg, cb);
        if (Selectable(buf))
            SetClipboardText(buf);
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            ImFormatString(buf, IM_ARRAYSIZE(buf), xorstr_( "#%02X%02X%02X%02X") , cr, cg, cb, ca);
            if (Selectable(buf))
                SetClipboardText(buf);
        }
        EndPopup();
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_picker && !allow_opt_alpha_bar) || !BeginPopup( xorstr_( "context") ))
        return;
    ImGuiContext& g = *GImGui;
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable( xorstr_( "##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags & ImGuiColorEditFlags_PickerMask_);
            SetCursorScreenPos(backup_pos);
            ImVec4 previewing_ref_col;
            base::memory::_memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4( xorstr_( "##previewing_picker"), &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags( xorstr_( "Alpha Bar"), &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    else
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration( xorstr_( ">"), NULL);
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        ImGuiID close_button_id = GetIDWithSeed( xorstr_( "#CLOSE") , NULL, id);
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            if (plot_type == ImGuiPlotType_Lines)
                SetTooltip( xorstr_( "%d: %8.4g\n%d: %8.4g") , v_idx, v0, v_idx + 1, v1);
            else if (plot_type == ImGuiPlotType_Histogram)
                SetTooltip( xorstr_( "%d: %8.4g"), v_idx, v0);
            idx_hovered = v_idx;
        }
...
{
    Text( xorstr_( "%s: %s"), prefix, (b ? xorstr_( "true" ) : xorstr_( "false") ));
}

void ImGui::Value(const char* prefix, int v)
{
    Text( xorstr_( "%s: %d"), prefix, v);
}
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
    ...
    {
        char fmt[64];
        ImFormatString(fmt, IM_ARRAYSIZE(fmt), xorstr_( "%%s: %s"), float_format);
        Text(fmt, prefix, v);
    }
    else
    {
        Text( xorstr_( "%s: %.3f"), prefix, v);
    }
}
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = BeginViewportSideBar( xorstr_( "##MainMenuBar" ), viewport, ImGuiDir_Up, height, window_flags);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(xorstr_(""), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups, ImVec2(w, 0.0f));
    ...
    else
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = Selectable( xorstr_( "" ), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = Selectable(xorstr_(""), selected, selectable_flags, ImVec2(w, 0.0f));
        PopStyleVar();
        RenderText(text_pos, label);
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        pressed = Selectable(xorstr_(""), false, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    base::memory::_memmove(dst_tab, src_tab, move_count * sizeof(ImGuiTabItem));
    *tab2 = item_tmp;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (ArrowButtonEx( xorstr_( "##<"), ImGuiDir_Left, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = -1;
    window->DC.CursorPos = ImVec2(x + arrow_button_size.x, tab_bar->BarRect.Min.y);
    if (ArrowButtonEx( xorstr_( "##>"), ImGuiDir_Right, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = +1;
    PopStyleColor(2);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo( xorstr_( "##v"), NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    tab_bar->TabsNames.append(label, label + base::memory::crc32_strlen(label) + 1);
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed( xorstr_( "#CLOSE") , NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
    ...
    if (text_clipped && g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > g.TooltipSlowDelay && IsItemHovered())
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
            SetTooltip( xorstr_( "%.*s") , (int)(FindRenderedTextEnd(label) - label), label);
